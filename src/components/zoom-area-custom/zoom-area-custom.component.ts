...
import { Content, Events, Gesture } from 'ionic-angular';
import { ZoomAreaCustomProvider } from './zoom-area-custom.provider';


@Component({
  selector: 'zoom-area-custom',
  templateUrl: 'zoom-area-custom.html',
  animations: [
...
export class ZoomAreaCustomComponent implements OnChanges {
  @ViewChild('zoomAreaCustomRoot') zoom;
  ...
  constructor(
    public zoomAreaProvider: ZoomAreaCustomProvider,
    public events: Events
  ) {
    this.zoomControlsState = 'hidden';
    events.subscribe('zoom:reset', () => {
      this.zoomReset();
    });
  }
  ...
  onTap(ev) {
    if (ev && ev.tapCount > 1) {
      ...
      this.setBounds();
      reset ? this.transform(0, 0) : this.transform();
    }
  }
  ...
  setBounds() {
    ...
    this.zoomConfig.max_x = overflow_x + scaled_x  + 20;
    this.zoomConfig.min_x = 0 + scaled_x;

    this.zoomConfig.max_y = -overflow_y + scaled_y;
